...
namespace ChunkedPathfinding
{
    public class Graph
    {
        ...
        public NativeNestedArray<BurstGraphPathfinder.GraphChunk> burstGraphChunks;

        public void ConvertChunks()
        {
            burstGraphChunks = new NativeNestedArray<BurstGraphPathfinder.GraphChunk>(graphChunks.Length, Allocator.Persistent);
            for (var i = 0; i < graphChunks.Length; i++)
            {
                if (graphChunks[i] != null)
                {
                    burstGraphChunks[i] = graphChunks[i].ToNative(Allocator.Persistent);
                }
            }
        }

        public List<Position> FindWithJob(int startX, int startY, int endX, int endY)
        {
            var startPos = GetVertexID(startX, startY);
            var start = new BurstGraphPathfinder.GraphNode[]
            {
                new BurstGraphPathfinder.GraphNode
                {
                    VertexID = startPos.ID,
                    ChunkID = startPos.ChunkID
                }
            };

            var endPos = GetVertexID(endX, endY);
            var end = new BurstGraphPathfinder.GraphNode[]
            {
                new BurstGraphPathfinder.GraphNode
                {
                    VertexID = endPos.ID,
                    ChunkID = endPos.ChunkID
                }
            };

            var result = new NativeList<BurstGraphPathfinder.GraphNode>(Allocator.TempJob);
            var finder = new BurstGraphPathfinder(burstGraphChunks, result, start, end, chunkSize, chunkSizeY, sizeY);

            var handle = finder.Schedule();
            handle.Complete();
            ...
            for (var i = 0; i < result.Length; i++)
            {
                ...
                {
                    var pathTile = result[i];
                    var chunk = GetChunk(pathTile.ChunkID);
                    int gridPosition = chunk.vertices[pathTile.VertexID].GridPosition;
                    x = gridPosition / sizeY;
                    y = gridPosition % sizeY;
